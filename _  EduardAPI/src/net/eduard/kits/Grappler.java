...
import net.eduard.net.GrapplerHook;

public class Grappler extends Kit {
	public Grappler() {
		setIcon(Material.LEASH,"Se mova mais rapido");
		add(Material.LEASH);
		setActiveCooldownOnPvP(true);
		setTime(5);
		setPrice(50 * 1000);
		new Click(Material.LEASH,new ClickEffect() {
						...
						if (e.getAction() == Action.LEFT_CLICK_AIR) {
							...
							if (hooks.containsKey(p)) {
								...
								if (hook.isHooked) {
									...
									Location target = hook.getBukkitEntity().getLocation();
									Vector velocity = GrapplerHook.moveTo(p.getLocation(), target, 0.5, 1.5, 0.5, 0.04,
											0.06, 0.04);
									p.setVelocity(velocity);
								} else {
									p.sendMessage("§6O gancho nao se prendeu em nada!");
								}
							}
						}
					} else {
						p.sendMessage("§6Voce esta em PvP e não pode usar o Kit!");
					}
				}
