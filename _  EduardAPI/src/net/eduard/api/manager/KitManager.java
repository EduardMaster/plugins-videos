...
public class KitManager extends EventsManager {
	...
	private Slot openKits = new Slot(
			ItemAPI.newItem(Material.CHEST, "§6§lSelecionar Kit"), 0);
	private Slot openShop = new Slot(
			ItemAPI.newItem(Material.EMERALD, "§6§lComprar Kit"), 8);
	private SimpleGui shopGui = new SimpleGui("§8Loja de Kits", 6);
	...
	public KitManager() {
		...
		kitsGui.setClick(new SimpleClick() {
			...
			public void onClick(InventoryClickEvent event, int page) {
				if (event.getWhoClicked() instanceof Player) {
					...
					if (kitsGui.getNextPage().equals(item)) {
						if (page == kitsGui.getPages())return;
		...
		shopGui.setClick(new SimpleClick() {
	...
	public void register(Player player) {
		...
		SimpleGui select = kitsGui.copy();
		SimpleGui shop = shopGui.copy();
	...
	public Map<Player, SimpleGui> getKitsGuis() {
		return kitsGuis;
	}
	public void setKitsGuis(Map<Player, SimpleGui> kitsGuis) {
		this.kitsGuis = kitsGuis;
	}
	public Map<Player, SimpleGui> getShopsGuis() {
		return shopsGuis;
	}
	public void setShopsGuis(Map<Player, SimpleGui> shopsGuis) {
	...
	public SimpleGui getShopGui() {
		return shopGui;
	}
	public void setShopGui(SimpleGui shopGui) {
		this.shopGui = shopGui;
	}
	public SimpleGui getKitsGui() {
		return kitsGui;
	}
	public void setKitsGui(SimpleGui kitsGui) {
