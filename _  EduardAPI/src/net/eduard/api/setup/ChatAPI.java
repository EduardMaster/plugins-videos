...
import net.eduard.internet.fanciful.FancyMessage;
...
public final class ChatAPI {
	...
	public static class ChatMessageListener implements Listener {
		...
		public void onCommand(PlayerCommandPreprocessEvent event) {
			...
			String cmd = ExtraAPI.getCmd(msg);
			for (ChatChannel channel : channels.values()) {
				if (ExtraAPI.startWith(cmd, "/" + channel.getName())) {
					channel.chat(event.getPlayer(), msg.replaceFirst(cmd, ""));
					event.setCancelled(true);
					break;
				}
				if (ExtraAPI.startWith(cmd, "/" + channel.getCommand())) {
					channel.chat(event.getPlayer(), msg.replaceFirst(cmd, ""));
					event.setCancelled(true);
					break;
				}
