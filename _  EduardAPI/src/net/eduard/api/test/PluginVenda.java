...
public class PluginVenda {
	...
	public PluginVenda(String line) {
	...
	public static void main(String[] args) {
		while (true) {
			try {
				PluginVenda venda = new PluginVenda();
	...
	public static List<PluginVenda> getVendas(String url) {
		List<PluginVenda> vendas = new ArrayList<>();
		try {
			...
			while (scan.hasNext()) {
				...
				vendas.add(new PluginVenda(line));
			}
			scan.close();
...
//		Bukkit.getConsoleSender().sendMessage("§aAutenticao do Plugin");
		Bukkit.getConsoleSender().sendMessage("§aHOST: §f" + BUKKIT_IP
				+ " §aINTERNET: §f" + SERVER_IP + " §aPORTA: §f" + PORTA);
		if (BUKKIT_IP.equals("localhost")) {
			Bukkit.getConsoleSender().sendMessage(
					"§aPlugin foi liberado para testes em 'localhost'");
			return;
		}
		List<PluginVenda> keys = getVendas(url);
		Bukkit.getConsoleSender()
				.sendMessage("§aVendas encontradas: §f" + keys.size());

		String error = "";
		String first = error;

		for (PluginVenda license : keys) {
			error = "";
			if (!(license.getPlugin().equals(plugin.getName()))) {
				continue;
			}
			if ((license.getHost().equals(SERVER_IP)
					|| license.getHost().equals(BUKKIT_IP))
