	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			for (int numeroGerado : numeros) {
				...
				if (numeroGerado < 10) {
					System.out.println("Você é muito cagado");
				}
				if (numeroGerado < 5) {
					System.out.println("Sua sorte é de ouro");
				}
				if (numeroGerado == 1) {
					System.out.println("Sua sorte é muito foda!");
				}
			}
			...
			if (resultadoDaChecagem == -1) {
				System.out.println("Voce é muito azarado não sorteou nenhum numero menor que 30");
			} else {
				System.out.println("Você até que é sortudo tirou o numero menor que 30: " + resultadoDaChecagem);
			}
			try {
				Thread.sleep(1000);
			} catch (InterruptedException e) {
