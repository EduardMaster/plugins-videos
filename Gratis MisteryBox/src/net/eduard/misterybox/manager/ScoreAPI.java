...
public class ScoreAPI {
	...
	public ScoreAPI setScoreboard(Player player) {
	...
	public ScoreAPI(String title) {
		...
		board = score.registerNewObjective("Scoreboard", "API");
		board.setDisplaySlot(DisplaySlot.SIDEBAR);
		health = score.registerNewObjective("HealthBar", "health");
		health.setDisplaySlot(DisplaySlot.BELOW_NAME);
	}
	public ScoreAPI set(int slot,String display) {
		slot = getSlot(slot);
		remove(slot);
	...
	public ScoreAPI empty(int slot) {
		slot = getSlot(slot);
		remove(slot);
	...
	public ScoreAPI remove(int slot) {
		if (lines.containsKey(slot)) {
			score.resetScores(lines.get(slot));
		}
		return this;
	}
	public ScoreAPI setHealthDisplay(String display) {
		health.setDisplayName(text(display,32));
		return this;
	}
	public ScoreAPI setTitle(String title) {
