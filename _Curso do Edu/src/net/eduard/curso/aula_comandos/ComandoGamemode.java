package net.eduard.curso.aula_comandos;
		...
		if (snd instanceof Player) {
			...
			if (args.length == 0) {
				if (p.getGameMode().equals(GameMode.CREATIVE)) {
					...
					p.sendMessage("§eSeu modo de jogo foi mudado sobrevivência. Enjoy");
				} else {
				...
				if (b != null) {
					if (args[0].equalsIgnoreCase("0")) {
						...
						snd.sendMessage("§eO modo de jogo de " + b.getName() + "foi mudado para sobrevivência");
					} else if (args[0].equalsIgnoreCase("1")) {
						b.setGameMode(GameMode.CREATIVE);
						snd.sendMessage("§eO modo de jogo de " + b.getName() + "foi mudado para sobrevivência");
					}
				} else {
					snd.sendMessage("§cEste jogador não esta online, ou não existe.");
				}
			}
		}
		return false;
